...
# total_data$Log_Chl <- log(total_data$Chl) # log transform the chlorophyll data, if required

# Extract the data that we want to work with - only data from the upper mixed layer:
# total_ML_split <- split(total_data, total_data$UpperMixed)  # Split the data set, if required
...
# Compare the linear mixed effects model and the linear model:
```{r}

# Mixed effects model
mean(resid(model1)) # mean
sigma(model1) # Residual standard error

# Linear model
mean(resid(model2)) # mean
sigma(model2)  # Residual standard error

# Model 1 has less deviation in the residuals, while both residuals means are close to zero.

# Compare AIC and BIC values:
anova(model1, model2)

# The results from this test suggest that the mixed effects model (model1) is a better fit.
...
# Utilize bootsrapping to test the model coefficients and accuracy:
```{r}
# Bootstrap is a method of random sampling with replacement. Linear regression relies on several assumptions, and the coefficients of the formulas are presumably normally distributed. It shows that on average if we repeated the experiment thousands and thousands of times, the line would be in confidence intervals.

# Predict values:
pred <- predict(model1, re.form = NA)

# Bootstrap Confidence Intervals:
boot <- bootMer(model1, predict, nsim = 100, re.form = NA)
    std.err <- apply(boot$t, 2, sd)
    CI.lo <- pred - std.err*1.96
    CI.hi <- pred + std.err*1.96

# Plot:
ggplot(UpperMixed_data, aes(x = Log_Chl, y = TEP)) +
    geom_line(aes(y = pred), size=2) +
    geom_line(aes(x = Log_Chl, y = TEP), alpha = 0.2) +
    geom_ribbon(aes(ymin = CI.lo, ymax = CI.hi), size=2, alpha = 0.03, linetype = 1)
...
# val_model1 <- lm(TEP ~ Log_Chl + Temperature, data = train.data1) # Alternative
...
ggarrange(Val_plot, Res_model1, H_model1, ncol = 3)
```
...
# Validation models:
val_model2 <- lmer(TEP ~ Log_Chl + Temperature + (1|Season), data = train.data2)
val_model3 <- lmer(TEP ~ Log_Chl + Temperature + (1|Season), data = train.data3)
val_model4 <- lmer(TEP ~ Log_Chl + Temperature + (1|Season), data = train.data4)
val_model5 <- lmer(TEP ~ Log_Chl + Temperature + (1|Season), data = train.data5)

# Alternative model options:
# val_model2 <- lm(TEP ~ Log_Chl + Temperature, data = train.data2)
# val_model3 <- lm(TEP ~ Log_Chl + Temperature, data = train.data3)
# val_model4 <- lm(TEP ~ Log_Chl + Temperature, data = train.data4)
# val_model5 <- lm(TEP ~ Log_Chl + Temperature, data = train.data5)

# Get predictions for the validation model training and testing sets:
test.data2$predict2 <- predict(val_model2, test.data2, type = "response")
test.data3$predict3 <- predict(val_model3, test.data3, type = "response")
test.data4$predict4 <- predict(val_model4, test.data4, type = "response")
test.data5$predict5 <- predict(val_model5, test.data5, type = "response")
...
# Temperature:
Temp <- ggplot(plot_data, aes(x = Temperature, y = TEP, shape = Season)) +
  facet_wrap(~Season, nrow = 3) + 
  geom_point(alpha = 0.7) + 
  geom_smooth(method = 'lm') +
  ylab("TEP (\U003BCg XG eq/L)") +
  xlab("Temperature (\u00B0C)") +
  theme_classic() +
  theme(
...
# Chlorophyll:
Chl <- ggplot(plot_data, aes(x = Log_Chl, y = TEP, shape = Season)) +
  facet_wrap(~Season, nrow = 3) + 
  geom_point(alpha = 0.7) + 
  geom_smooth(method = 'lm') + 
  ylab("") +
  xlab("Log Chlorophyll a (\U003BCg/L)") +
  theme_classic() +
  theme(
  strip.background = element_blank(),
  strip.text.x = element_blank(),
...
# Input python dataframe composed of predictor variables from NASA Satellites:
```{r}

# Use the arrow and feather packages:
df_Spring_2019 <- arrow::read_feather('May_June_2019_Predictions.feather')
df_Spring_2020 <- arrow::read_feather('May_June_2020_Predictions.feather')
df_Spring_2021 <- arrow::read_feather('May_June_2021_Predictions.feather')

```

# Make predictions on the python Satellite dataframe:
```{r}

# Spring 2019
Predictions_Spring_2019 <- predict(model1, df_Spring_2019, type = "response")
Predictions_Spring_2019 <- as.data.frame(Predictions_Spring_2019)  # Create a data frame of predictions

# Spring 2020
Predictions_Spring_2020 <- predict(model1, df_Spring_2020, type = "response")
Predictions_Spring_2020 <- as.data.frame(Predictions_Spring_2020)  # Create a data frame of predictions

# Spring 2021
Predictions_Spring_2021 <- predict(model1, df_Spring_2021, type = "response")
Predictions_Spring_2021 <- as.data.frame(Predictions_Spring_2021)  # Create a data frame of predictions

# Save the predictions back into a feather file, to be imported back into python:
feather::write_feather(Predictions_Spring_2019, 'May_June_2019_Predictions.feather')
feather::write_feather(Predictions_Spring_2020, 'May_June_2020_Predictions.feather')
feather::write_feather(Predictions_Spring_2021, 'May_June_2021_Predictions.feather')
