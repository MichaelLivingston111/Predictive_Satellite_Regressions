...
ggplot(total_data, aes(Log_Chl, TEP, colour = Season)) +
         geom_point()
...
# Step-wise Regressions models: Examine the most influential predictor variables:
```{r}
...
# Create a series of predictive models based off the informaiton from stepwise selection above:
```{r}
...
AIC(Satellite_All_lmer, Satellite_lm, base_model, model_2, model_3, model_4)
AICc(Satellite_All_lmer, Satellite_lm, base_model, model_2, model_3, model_4)

# Linear model 'Satellite lm' is the preffered model based of AIC scores.
...
# The above loop generated 30 different regression models with model coefficients, y intercepts and predictions, specified below for each of the models created: 
...
colnames(MAE) <- c('Chl','Chl + Temperature', 'Chl + Temperature + POC', 
                'Chl + Temperature + POC + Season', 'Chl + Temperature + POC + PAR')
...
data <- total_data %>% select(TEP, Temperature, Season, Log_Chl, POC, 
                              Avg_PAR, SST, Region, Summer_2021_LP, Spring_2021_LP)
...
CV <- ggplot() +
  ...
  geom_point(aes(x = data_Else$TEP, y = data_Else$predict), colour = 'black', alpha = 0.3) +  # Training data

  geom_point(aes(x = data_spring_2021$TEP, data_spring_2021$predict), colour = 'blue', fill = 'blue', shape = 23) +  # Test data
...
# Modify the vis.gam function to include my colours!
```{r}
