...
for (i in 1:50) {
  ...
  model_bootstrap_forward <- lm(TEP ~ Log_Chl + Temperature + DO + POC + Avg_PAR + Nitrate +
                                  factor(Season), data = train.data1)  # forwards selection
  
  model_bootstrap_backward <- lm(TEP ~ Log_Chl + Temperature + DO + POC + Avg_PAR + Sigma +
                                   factor(Season), data = train.data1)  # backwards selection
  
  Theo_Model3 <- lmer(TEP ~ Log_Chl + Temperature + Log_MLD + POC + (1|Season), 
                    data = train.data1)  # Complex Satellite

  Theo_Model4 <- lmer(TEP ~ Log_Chl + Temperature + (1|Season), 
                    data = train.data1)  # Satellite

  Theo_Model5 <- lm(TEP ~ Log_Chl + DO + Nitrate + factor(Season), 
                  data = train.data1)  # Simplified

  Theo_Model6 <- lm(TEP ~ Log_Chl + DO + Nitrate, 
                  data = train.data1)  # Simplified
...
forward_CV <- ggplot(predictions, aes(x = sample_train, y = sample_predict)) +
    ...
    ggtitle("A: Chl (log) + Temperature + DO + POC + PAR + 
            Nitrate + factor(Season)") +
    ylim(0, 200) +
    xlim(0, 200) +
    ylab("") +
    xlab("") +
...
backward_CV <- ggplot(predictions, aes(x = sample_train, y = sample_predict2)) +
    geom_point(alpha = 1, colour = "blue") +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
    ggtitle("B: Chl (log) + Temperature + DO + POC + PAR + 
            Density + factor(Season)") +
    ylim(0, 200) +
    xlim(0, 200) +
    ylab("") +
    xlab("") +
...
Model3 <- ggplot(predictions, aes(x = sample_train, y = sample_predict_3)) +
    ...
    geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
    ggtitle("E: Chl (log) + Temperature + POC + MLD + (1|Season)
            ") +
    ylim(0, 200) +
    xlim(0, 200) +
    ylab("") +
...
Model4 <- ggplot(predictions, aes(x = sample_train, y = sample_predict_4)) +
    geom_point(alpha = 1, colour = "red") +
    geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
    ggtitle("F: Chl (log) + Temperature + (1|Season)
            ") +
    ylim(0, 200) +
    xlim(0, 200) +
    ylab("") +
...
Model5 <- ggplot(predictions, aes(x = sample_train, y = sample_predict_5)) +
    ...
    geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
    ggtitle("C: Chl (log) + Nitrate + DO + factor(Season)
            ") + 
    ylim(0, 200) +
    xlim(0, 200) +
    ylab("") +
...
Figure <- ggarrange(forward_CV, Model5, Model3, 
          backward_CV, Model6, Model4, nrow = 2, ncol = 3)
...
Theo_Model3 <- lmer(TEP ~ Log_Chl + Temperature + POC + Log_MLD + (1|Season), 
                    data = UpperMixed_data)  # Complex Satellite
