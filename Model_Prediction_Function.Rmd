...
global_data <- read.csv("Total_data_DBO.csv")
...
Predict_fn_regional <- function(df) {
  ...
  regional_data <- total_data %>% select(TEP, Log_Chl, Temperature, Log_MLD, MLD, Season, POC, Avg_PAR)
  
  # Create the model:
  regional_model <- lm(TEP ~ Log_Chl + Temperature + POC + Season, data = UpperMixed_data) 
...
# Upload all the satellite data from 2021 to be used as predictors:
# 8 day spring 2021 Global:
df_Spring_8D_2021 <- arrow::read_feather('May_2021_LineP.feather')

# 8 day summer 2021 Global:
df_Summer_8D_2021 <- arrow::read_feather('Aug_2021_LineP.feather')
