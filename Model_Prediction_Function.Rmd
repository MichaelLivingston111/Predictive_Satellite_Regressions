...
Predict_fn_regional <- function(df) {
  ...
  # Create the model:
  regional_model <- lm(TEP ~ Log_Chl + Temperature + POC + Season, 
                       data = regional_data) 
...
Predict_fn_global <- function(df) {
  ...
  # Select the relevant data for the model:
  global_data <- global_data %>% select(TEP, Season, Log_Chl, POC, Summer, 
                                        Arctic, Area, Temperature)
  global_data <- na.omit(global_data)
  
  # Create the model:
  global_model <- lm(TEP ~ Log_Chl + POC + Temperature + Arctic, 
                     data = global_data)
